...
import { Briefcase, TrendingUp, ArrowRight, Sparkles } from 'lucide-react';
      ...
      {allJobs?.length > 0 ? (
        <>
          ...
          {allJobs?.length > 6 && (
            <div className="text-center mt-12">
              <Link to="/jobs" onClick={handleViewAllJobs}>
        ...
        <div className="text-center py-20 bg-gradient-to-br from-slate-50 to-purple-50/30 rounded-3xl border-2 border-dashed border-purple-200">
          ...
          <Link 
            to="/jobs" 
            onClick={handleViewAllJobs}
            className="inline-block mt-6 px-8 py-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 transition-colors"
          >
            Browse All Jobs
          </Link>
        </div>
